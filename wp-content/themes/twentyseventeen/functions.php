...
require get_template_directory() . '/nfl_sedule.php';

// Function to get the customer count
function get_customer_count() {
    $customer_count = get_option('customer_purchase_count', 0); // Retrieve count from options
    return $customer_count;
}

// Function to increment customer count
function increment_customer_count() {
    $customer_count = get_option('customer_purchase_count', 0); // Get current count
    $customer_count++; // Increment count
    update_option('customer_purchase_count', $customer_count); // Update count in options
}
add_action('woocommerce_thankyou', 'custom_payment_confirmation', 999);

function custom_payment_confirmation($order_id) {
    increment_customer_count(); // Increment count after displaying confirmation
    $customer_count = get_customer_count();
    $order = wc_get_order($order_id);

    if ($customer_count % 3 != 0) {
        // Redirect to homepage and show popup
        $order_date = $order->get_date_created()->date('F j, Y');
        $order_email = $order->get_billing_email();
        $order_total = $order->get_formatted_order_total();
        $payment_method = $order->get_payment_method_title();

        WC()->session->set('show_popup', true);
        WC()->session->set('order_id', $order->get_order_number());
        WC()->session->set('payment_status', $order->get_status());
        WC()->session->set('order_date', $order_date);
        WC()->session->set('order_email', $order_email);
        WC()->session->set('order_total', $order_total);
...
function add_popup_script() {
    if (WC()->session->get('show_popup')) {
        ?>
        <script type="text/javascript">
            document.addEventListener("DOMContentLoaded", function() {
                var orderId = "<?php echo WC()->session->get('order_id'); ?>";
                var paymentStatus = "<?php echo WC()->session->get('payment_status'); ?>";
                var orderDate = "<?php echo WC()->session->get('order_date'); ?>";
                var orderEmail = "<?php echo WC()->session->get('order_email'); ?>";
                var orderTotal = '<?php echo WC()->session->get("order_total"); ?>';
                var paymentMethod = "<?php echo WC()->session->get('payment_method'); ?>";
                ...
                popup.innerHTML = '<img src="http://local.btwordpress.com/wp-content/uploads/2024/07/thumup-removebg-preview.png" class="tumbup"><h1 class="order_received">Bestellung abgeschlossen</h1><h2 class="tmessage">Vielen Dank. Deine Bestellung ist eingegangen.</h2>' +
                    '<div class="order_id">Bestellnummer: ' + orderId + '</div>' +
                    '<div class="payment_status">Zahlungsstatus: ' + paymentStatus + '</div>' +
                    '<div class="order_date">Datum: ' + orderDate + '</div>' +
                    '<div class="order_email">Email: ' + orderEmail + '</div>' +
                    '<div class="order_total">Gesamt: ' + orderTotal + '</div>' +
                    '<div class="payment_method">Zahlungsmethode: ' + paymentMethod + '</div>' +
                ...
                <?php
                WC()->session->set('show_popup', false);
                WC()->session->set('order_id', null);
                WC()->session->set('payment_status', null);
                WC()->session->set('order_date', null);
                WC()->session->set('order_email', null);
                WC()->session->set('order_total', null);
        ...
        <style type="text/css">
            ...
            div#custom-popup .order_id, 
            div#custom-popup .payment_status,
            div#custom-popup .order_date,
            div#custom-popup .order_email,
            div#custom-popup .order_total,
            div#custom-popup .payment_method {
                font-size: 1.1rem;
                line-height: 34px;
                font-weight: 600;
                color: rgb(51, 51, 51);
